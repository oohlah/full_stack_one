...
export const userApi = {
  find: {
    ...
    handler: async function (request: Request, h: ResponseToolkit): Promise<User[] | Boom.Boom> {
      try {
  ...
  create: {
    ...
    handler: async function (request: Request, h: ResponseToolkit): Promise<User | Boom.Boom | ResponseObject> {
      try {
      ...
      try {
        ...
        const user = await db.userStore.getUserByEmail(payload.email);
         console.log("USER FOUND:", user);
          console.log("PASSWORD CHECK:", user?.password, payload.password);
        if (!user) {
